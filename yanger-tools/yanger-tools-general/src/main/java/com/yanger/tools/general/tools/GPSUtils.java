...
public class GPSUtils {
    ...
    public static double[] calGCJ02toBD09(double latitude, double longitude) {
        double z = Math.sqrt(longitude * longitude + latitude * latitude) + 2.0E-5D * Math.sin(latitude * 52.35987755982988D);
        double theta = Math.atan2(latitude, longitude) + 3.0E-6D * Math.cos(longitude * 52.35987755982988D);
    ...
    public static boolean isOutOfChina(double latitude, double longitude, boolean precision) {
        if (precision) {
            return CHINA_POLYGON.stream().noneMatch((point) -> {
                return pointInPolygon(point, latitude, longitude);
            });
     ...
     * @param: latitude 维度
     * @param: latitude 经度
     * @return: com.yanger.tools.general.tools.GPSUtils.Point
     * @throws
     */
