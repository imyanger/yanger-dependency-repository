...
public interface SerializeEnum<T extends Serializable> {
    ...
    static <E extends Enum<E> & SerializeEnum<?>> E valueOf(Class<E> clazz, Serializable value) {
        String errorMessage = StrFormatter.format(
                "枚举类型转换错误: 没有找到对应枚举. value = {}, SerializeEnum = {}", value, clazz);
    ...
    static <T> T getEnumByOrder(Class<? extends Enum<?>> clz, Serializable finalValue) {
        ...
        if (getForName.isPresent()) {
            ...
            try {
                ...
                throw new BasicException(BasicException.DEFAULT_ERROR_CODE, "通过枚举下标查找枚举出错: [{}] 无法转换为下标", value);
            }
            // 使用下标匹配
            Optional<? extends Enum<?>> getForOrdinal = EnumUtils.getEnum(clz, e -> e.ordinal() == index);
            result = (T) getForOrdinal.orElseThrow(() -> BasicException.of("无法将 [{}] 转换为 [{}]", value, clz));
        }
        return result;
    }
