...
/**
 * @Description 在 NacosConfigBootstrapConfiguration 之前执行
 * @Author yanger
...
public class YangerNacosBootstrapConfiguration {
    ...
    public NacosPropertySourceLocator nacosPropertySourceLocator(@NotNull NacosConfigProperties nacosConfigProperties) {
        ...
        return new YangerNacosPropertySourceLocator(new NacosConfigManager(nacosConfigProperties));
    }
     ...
     */
    @Order(-1)
    public class YangerNacosPropertySourceLocator extends NacosPropertySourceLocator {
        /** Nacos config properties */
        ...
        /**
         * yanger nacos property source locator
         *
         * @param nacosConfigManager nacos config manager
         */
        YangerNacosPropertySourceLocator(NacosConfigManager nacosConfigManager) {
        ...
        public PropertySource<?> locate(@NotNull Environment environment) {
            ...
            YangerNacosProperties yangerNacosProperties = new YangerNacosProperties();
            Binder.get(environment).bind(YangerNacosProperties.PREFIX, Bindable.ofInstance(yangerNacosProperties));

            // 是否优先从 Nacos 中读取配置 (默认为 true)
            if (yangerNacosProperties.getEnableNacosConfig() == null) {
                yangerNacosProperties.setEnableNacosConfig(true);
            }
            // 是否自动向 nacos 创建配置 (默认为 false)
            if (yangerNacosProperties.getEnableAutoCreateConfig() == null) {
                yangerNacosProperties.setEnableAutoCreateConfig(false);
            }

            if (!yangerNacosProperties.getEnableAutoCreateConfig()) {
            ...
            if (!yangerNacosProperties.getEnableNacosConfig()) {
