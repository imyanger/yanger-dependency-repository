 ...
 */
public class BasicService<M extends BaseDao<T>, T> extends ServiceImpl<M, T> {
    ...
    public boolean saveIgnoreBatch(Collection<T> entityList) {
        return this.saveBatch(entityList, 1000, MybatisSqlMethod.INSERT_IGNORE_ONE);
    }
     ...
     */
    @Transactional(rollbackFor = Exception.class)
    public boolean saveReplaceBatch(Collection<T> entityList) {
        return this.saveBatch(entityList, 1000, MybatisSqlMethod.REPLACE_ONE);
    }
